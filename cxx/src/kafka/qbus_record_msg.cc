...
static const long RECODE_FILE_SIZE = 100 * 1024 * 1024;
...
void QbusRecordMsg::init() {
    std::auto_ptr<log4cplus::Layout> layout(new log4cplus::PatternLayout("%m %n"));
    if (NULL != layout.get()) {
        log4cplus::SharedAppenderPtr fileAppender(new log4cplus::DailyRollingFileAppender(
            RECORD_DEFAULT_LOG_FILE_NAME, log4cplus::HOURLY, true, RECORD_FILE_BACKUP_INDEX));
