...
int main(int argc, char* argv[]) {
  ...
  if (NULL != consumer_handle) {
    if (QBUS_RESULT_OK ==
        ...
        InitQbusConsumer(consumer_handle, argv[3], "./consumer.log",
                         "./consumer.config", QbusConsumerDeliveryMsg)
#else

        InitQbusConsumerEx(consumer_handle, argv[3], "./consumer.log",
                           "./consumer.config", callback_info)
#endif
    ) {

      if (QBUS_RESULT_OK ==
          QbusConsumerSubscribeOne(consumer_handle, argv[2], argv[1])) {
