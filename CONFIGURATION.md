...
log.level    		            #sdk的日志等级  默认info
enable.rdkafka.log  	      #底层rdkafka的日志 在此选项打开时 需要在global配置块下加入debug=all配置项（具体使用方式见rdkafka配置文档）  默认不打开 在生成时在当下文件夹./rdkafka.log
broker.list                 #broker地址 可在配置文件中配置
manual.commit.offset.async  #手动提交offset 默认未开启		
